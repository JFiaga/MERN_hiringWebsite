...
const GigsCard = ({firstName, lastName,specialisation,city,desc, id,isCertified}: Props) => {
  ...
  return (
    <div className='text-black  bg-white   flex flex-col items-center space-y-4 w-[90%] sm:max-w-[350px] h-[600px]shadow-sm border border-black/10 rounded-md relative   overflow-hidden px-4 ml-4 mb-4 pb-5'>
      <div className='absolute w-[100px] h-[100px] rounded-full bg-primary top-[-5%] left-[-15%] '></div>
        ...
        {isCertified ? <div><Certified bool={true} value='Certified'/></div> : <div><Certified value='Not Certified' bool={false}/></div>}
        <div className='flex flex-col items-center'>
          <span className='font-bold'>{firstName} {lastName}</span>
        <span className='font-semibold'><span className='text-primary'>{specialisation}</span> developper</span>
        <div className='flex justify-center items-center space-x-1 flex-wrap'>
          <GrLocation className='text-xl mr-1'/>
          <span>{city}</span>
         
        </div>
        <span className='text-sm text-gray-700 text-center'>{desc.substring(0,80)}</span>
        </div>
        <div className='flex w-full text-white font-semibold '>
       
          <Link to={`/profile/${id}`} className='w-full bg-primary text-center capitalize rounded-md py-3 transition-all duration-500 border border-transparent hover:border-primary hover:bg-transparent hover:text-primary'> view Profile</Link>
        </div>
